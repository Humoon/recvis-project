...
# SMPL joint names
theta_names = [
    'Left_Hip', 'Right_Hip', 'Waist', 'Left_Knee', 'Right_Knee', 'Upper_Waist',
    'Left_Ankle', 'Right_Ankle', 'Chest', 'Left_Toe', 'Right_Toe', 'Base_Neck',
    'Left_Shoulder', 'Right_Shoulder', 'Upper_Neck', 'Left_Arm', 'Right_Arm',
    'Left_Elbow', 'Right_Elbow', 'Left_Wrist', 'Right_Wrist', 'Left_Finger',
    'Right_Finger'
]
...
for k in keys:
    ...
    rotation_matrices = tf.Session().run(
        batch_rodrigues(
            tf.convert_to_tensor(all_theta[3:72 + 3].reshape(-1, 3),
                                 dtype=tf.float32)))
    quater_rot = []
    ...
    for k in range(len(theta_wanted)):
        if theta_wanted[k] in oneD_theta:
            oneD_rot = np.linalg.norm(all_theta[3:72 + 3].reshape(
                -1, 3)[theta_names.index(theta_wanted[k])])
            #check here again for the sign
            l_output.append(-np.sign(all_theta[3:72 + 3].reshape(
                -1, 3)[theta_names.index(theta_wanted[k])][-1]) * oneD_rot)
        else:
...
json.dump(json_mimic, open('deepmimic.json', 'w'), indent=4)
