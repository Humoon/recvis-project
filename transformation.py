...
def rotation_from_matrix(matrix):
    ...
    if abs(direction[2]) > 1e-8:
        sina = (R[1, 0] +
                (cosa - 1.0) * direction[0] * direction[1]) / direction[2]
    elif abs(direction[1]) > 1e-8:
        sina = (R[0, 2] +
                (cosa - 1.0) * direction[0] * direction[2]) / direction[1]
    else:
        sina = (R[2, 1] +
                (cosa - 1.0) * direction[1] * direction[2]) / direction[0]
...
def projection_matrix(point,
    ...
    if perspective is not None:
        ...
        perspective = numpy.array(perspective[:3],
                                  dtype=numpy.float64,
                                  copy=False)
...
def superimposition_matrix(v0, v1, scale=False, usesvd=True):
    ...
    return affine_matrix_from_points(v0,
                                     v1,
                                     shear=False,
                                     scale=scale,
                                     usesvd=usesvd)
