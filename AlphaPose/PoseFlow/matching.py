...
def orb_matching(img1_path, img2_path, vidname, img1_id, img2_id):
    ...
    for i, m_n in enumerate(matches):
        if len(m_n) != 2:
            ...
            ret = fd.write(
                "%d %d %d %d %f \n" %
                (kp1[m_n[0].queryIdx].pt[0], kp1[m_n[0].queryIdx].pt[1],
...
if __name__ == '__main__':
    ...
    for vidname in tqdm(sorted(vidnames)):
        ...
        for imgname in imgnames[:-1]:
            ...
            if len_name == 5:
                img2 = os.path.join(
                    vidname, "%05d.jpg" % (int(imgname.split(".")[0]) + 1))
            else:
                img2 = os.path.join(
                    vidname, "%08d.jpg" % (int(imgname.split(".")[0]) + 1))
            ...
            if not os.path.exists(
                    cor_file) or os.stat(cor_file).st_size < 1000:
                if args.orb:
                    # calc orb matching
                    orb_matching(img1, img2, vidname, img1_id, img2_id)
                else:
                    # calc deep matching
                    cmd = "./deepmatching/deepmatching %s %s -nt 10 -downscale 3 -out %s/%s_%s.txt > cache" % (
                        img1, img2, vidname, img1_id, img2_id)
                    os.system(cmd)
