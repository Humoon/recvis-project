...
def cal_pose_iou_dm(all_cors, pose1, pose2, num, mag):
    ...
    for ids in range(len(pose1)):
        pose1_box = [
            pose1[ids][0] - mag, pose1[ids][0] + mag, pose1[ids][1] - mag,
            pose1[ids][1] + mag
        ]
        pose2_box = [
            pose2[ids][0] - mag, pose2[ids][0] + mag, pose2[ids][1] - mag,
            pose2[ids][1] + mag
        ]
...
def best_matching_hungarian(all_cors, all_pids_info, all_pids_fff,
                            track_vid_next_fid, weights, weights_fff, num,
                            mag):
    ...
    for pid1 in range(box1_num):
        ...
        for pid2 in range(1, track_vid_next_fid['num_boxes'] + 1):
            ...
            pose_iou_dm = cal_pose_iou_dm(all_cors, box1_pose, box2_pose, num,
                                          mag)
            pose_iou = cal_pose_iou(box1_pose, box2_pose, num, mag)
            if box1_fff:
                grade = cal_grade([
                    dm_iou, box_iou, pose_iou_dm, pose_iou, box1_score,
                    box2_score
                ], weights)
            else:
                grade = cal_grade([
